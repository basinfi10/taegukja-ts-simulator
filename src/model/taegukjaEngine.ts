...
export class TaegukjaEngine {
  ...
  getSnapshot(): SimulationSnapshot {
    return {
      ...
      metrics: { ...this.lastMetrics, forceMetrics: { ...this.lastMetrics.forceMetrics }, forceDecomposition: { ...this.lastMetrics.forceDecomposition }, eventCycleMetrics: { ...this.lastMetrics.eventCycleMetrics }, priorityMetrics: { ...this.lastMetrics.priorityMetrics }, scale: { ...this.lastMetrics.scale } }
    };
  }
  ...
  private processEventCirculation(phaseDelta: number[], energyDelta: number[], boundDelta: number[], scaledDt: number): void {
    ...
    const maxPulses = this.nodes.length > 2200 ? 160 : 240;
    ...
    for (const edge of this.edges) {
      ...
      const drive = this.config.eventCouplingStrength * edge.resonance * edge.impedanceMatch * (0.35 + edge.binding + edge.circulationScore * 0.65) * (0.35 + activity) * closureGate;
      ...
      if (drive > this.config.eventTriggerThreshold || (edge.pulseStrength > this.config.eventTriggerThreshold * 0.82 && drive > this.config.eventTriggerThreshold * 0.45)) {
        const forward = Math.sin(a.eventClock + edge.triggerDelay - b.eventClock) >= 0;
        const src = forward ? a : b;
        const dst = forward ? b : a;
        const intensity = clamp01((drive - this.config.eventTriggerThreshold * 0.45) * 0.78);
        ...
        if (emitted < maxPulses) {
          const t = clamp01(edge.pulseStrength);
          newPulses.push({
            id: this.pulseSeq++,
            from: src.id,
            to: dst.id,
    ...
    const aged = this.eventPulses
      .map((p) => ({ ...p, age: p.age + 1, intensity: p.intensity * 0.88 }))
      .filter((p) => p.age < 28 && p.intensity > 0.035);
    this.eventPulses = [...newPulses, ...aged].slice(0, maxPulses);

    this.detectEventCycles(scaledDt);
  }
  ...
  private samplePathStats(): { avg: number; max: number; unreachable: number } {
    ...
    return { avg: count ? sum / count : Infinity, max, unreachable: miss / samples };
  }
  ...
  private physicalScale(): PhysicalScaleInfo {
    ...
    const visibleNodesPerParticle = Math.max(1, this.config.nodesPerParticleBase || 1000);
    ...
    return { planckLengthM, planckEnergyJ, particleEffectiveRadiusM, targetRestEnergyJ, linearRatio, realTaegeukjaPerParticle, visibleNodesPerParticle, realTaegeukjaPerVisibleNode, totalParticleCapacityInView, effectiveEnergyPerRealTaegeukjaJ, effectiveEnergyOccupancyOfPlanck, representativeEnergyPerNodeJ, energyUnitJ, massUnitKg, completeParticleFraction: this.config.completeParticleFraction, stableParticleAge: this.config.stableParticleAge, adaptiveNodeRendering: this.config.adaptiveNodeRendering, disableResonanceTerm: this.config.disableResonanceTerm, disableImpedanceTerm: this.config.disableImpedanceTerm, disableColorTerm: this.config.disableColorTerm, disableEnergyCorrection: this.config.disableEnergyCorrection, randomBondModel: this.config.randomBondModel };
  }
